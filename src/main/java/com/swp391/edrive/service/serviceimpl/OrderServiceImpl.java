...
public class OrderServiceImpl implements OrderService {
    ...
    private OrderResponse mapToOrderResponse(Order order) {
        ...
        // Tính tiền cọc 7%
        BigDecimal depositAmount = order.getTotalPrice()
                .multiply(new BigDecimal("0.07"))
                .setScale(0, RoundingMode.HALF_UP);
        res.setDepositAmount(depositAmount);

            // Tính số tiền còn lại phải thanh toán (93%)
        BigDecimal remainingAmount = order.getTotalPrice().subtract(depositAmount);
        ...
        if (order.getOrderItems() != null) {
            res.setOrderItems(
                    order.getOrderItems().stream().map(item -> {
                        ...
                        itemRes.vehicleImageUrl = item.getVehicle().getImageUrl();
    ...
    private OrderSummaryResponse buildOrderSummaryResponse(Order order, List<OrderItem> orderItems) {
        ...
        // Tính tiền cọc 7%
        response.depositAmount = order.getTotalPrice()
                .multiply(new BigDecimal("0.07"))
                .setScale(0, RoundingMode.HALF_UP);
        ...
        for (OrderItem item : orderItems) {
            ...
            itemResponse.vehicleVersion = item.getVehicle().getVersion();
            itemResponse.vehicleImageUrl = item.getVehicle().getImageUrl();
